...
ax.text(6.1,4.05,"CUDA Python",ha="center",va="center",fontsize=14,fontweight="bold",color=INK)
...
for i,(name,blurb,fc,ec) in enumerate(libs):
    ...
    ax.text(x+w/2,y+h-0.34,name,ha="center",va="center",fontsize=12.5,fontweight="bold",
            color=INK, family="monospace")
    wrapped="\n".join(textwrap.wrap(blurb,14))
    ax.text(x+w/2,y+0.95,wrapped,ha="center",va="center",fontsize=12.5,color="#2c3b41")
    ax.add_patch(FancyArrowPatch((6.1,3.7),(x+w/2,y+h),arrowstyle="-|>",mutation_scale=12,lw=1.3,color=GREY))
# highlight this talk
ci=2; cx=x0+ci*(w+gap)+w/2
ax.text(cx,0.2,"this talk",ha="center",fontsize=9.5,color=GREEN,fontweight="bold")
