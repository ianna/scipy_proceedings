...
data = json.load(open(os.path.join(DIM, "dimuon_timeline.json")))
...
for which, kind, y, lane_lbl, color, group in lanes:
    ...
    ax.text(-xmax*0.008, y, lane_lbl, ha="right", va="center", fontsize=10, color=INK)
...
ax.text(-xmax*0.135, 2.5, "before", ha="center", va="center", fontsize=12, fontweight="bold", color=RED, rotation=90)
ax.text(-xmax*0.135, 0.5, "after",  ha="center", va="center", fontsize=12, fontweight="bold", color=GREEN, rotation=90)
...
ax.set_ylim(-0.7, 3.9)
ax.set_yticks([])
ax.set_xlabel("GPU time (ms)  ·  actual nsys trace", fontsize=10.5, color=INK)
