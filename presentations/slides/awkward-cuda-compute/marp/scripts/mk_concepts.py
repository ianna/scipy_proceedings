...
def box(ax,x,y,w,h,title,sub=None,fc=LGREY,ec=GREY,fs=12,tc=INK,bold=True):
    ...
    if sub: ax.text(x+w/2,y+h/2-0.25,sub,ha="center",va="center",fontsize=9,color="#5a6b70")
...
box(ax,3.3,2.5,3.4,0.7,"CUDA Python",fc="#e8eef2",ec=INK,fs=13)
...
for i,(nm,fc,ec) in enumerate(kids):
    x=x0+i*(w+gap); box(ax,x,0.7,w,0.8,nm,fc=fc,ec=ec,fs=10.5,bold=(nm=="cuda.compute"))
    arr(ax,(5,2.5),(x+w/2,1.5),c=GREY,lw=1.4)
ax.text(x0+2*(w+gap)+w/2,0.45,"↑ this talk",ha="center",fontsize=9,color=GREEN,fontweight="bold")
...
box(ax,0.2,0.9,3.0,1.0,"NumPy · CuPy","PyTorch · JAX",fc=LBLUE,ec=BLUE,fs=12)
box(ax,3.5,0.9,3.0,1.0,"cuda.compute","composable primitives",fc=LGREEN,ec=GREEN,fs=12.5)
box(ax,6.8,0.9,3.0,1.0,"CUDA C++","CUB · Thrust",fc=LGREY,ec=GREY,fs=12)
arr(ax,(3.25,1.4),(3.48,1.4)); arr(ax,(6.55,1.4),(6.78,1.4))
ax.text(0.2,0.45,"◀ higher level (arrays)",fontsize=10,color="#5a6b70")
ax.text(9.8,0.45,"lower level (kernels) ▶",fontsize=10,color="#5a6b70",ha="right")
...
for i,(nm,fc,ec) in enumerate(seq):
    box(ax,x,0.8,w,0.9,nm,fc=fc,ec=ec,fs=10)
    if i<len(seq)-1: arr(ax,(x+w,1.25),(x+w+gap,1.25))
    x+=w+gap
ax.text(5,0.4,"compiled once per (dtype, op) specialization — then cached",ha="center",fontsize=9.5,color="#5a6b70",style="italic")
...
ax.text(0.15,2.3,"JIT",fontsize=11,fontweight="bold",color=INK)
box(ax,1.2,1.8,2.4,0.7,"first call",sub="compile",fc=LRED,ec=RED,fs=10)
box(ax,4.0,1.8,2.4,0.7,"cached",sub="reused in session",fc=LGREY,ec=GREY,fs=10)
arr(ax,(3.6,2.15),(4.0,2.15))
ax.text(0.15,1.1,"AoT",fontsize=11,fontweight="bold",color=GREEN)
box(ax,1.2,0.55,2.4,0.7,"serialize",sub="compiled kernel → disk",fc=LGREEN,ec=GREEN,fs=10)
box(ax,4.0,0.55,2.4,0.7,"deserialize",sub="near-instant load",fc=LGREEN,ec=GREEN,fs=10)
box(ax,6.8,0.55,3.0,0.7,"no compile at run",fc=LBLUE,ec=BLUE,fs=10)
arr(ax,(3.6,0.9),(4.0,0.9),c=GREEN); arr(ax,(6.4,0.9),(6.8,0.9),c=GREEN)
ax.text(9.85,0.2,"ref: NVIDIA/cccl PR #9732",ha="right",fontsize=8.5,color="#7c8b90")
fig.savefig(f"{OUT}/aot.png"); plt.close(fig)

# 5.4 lazy array
fig,ax=plt.subplots(figsize=(10,2.7)); ax.set_xlim(0,10); ax.set_ylim(0,2.7); ax.axis("off")
ax.text(0.15,2.4,"today (eager)",fontsize=10.5,fontweight="bold",color=RED)
for i,op in enumerate(["op","op","op"]):
    x=1.4+i*2.3
    box(ax,x,1.7,1.4,0.6,op,fc=LGREY,ec=GREY,fs=10)
    box(ax,x,2.35,1.4,0.001,"",fc="none",ec="none")
    ax.text(x+0.7,1.5,"↓ kernel",ha="center",fontsize=8,color=RED)
ax.text(0.15,0.95,"future (lazy)",fontsize=10.5,fontweight="bold",color=GREEN)
box(ax,1.4,0.5,3.3,0.7,"expression graph",sub="capture whole chain",fc=LGREEN,ec=GREEN,fs=11)
box(ax,5.6,0.5,3.6,0.7,"one fused pipeline",sub="fuse ACROSS ops",fc=LBLUE,ec=BLUE,fs=11)
arr(ax,(4.7,0.85),(5.6,0.85),c=GREEN)
ax.text(9.85,0.15,"ref: scikit-hep/awkward #4173",ha="right",fontsize=8.5,color="#7c8b90")
