...
d=json.load(open(os.path.join(HERE,"bench_ak_argmin.json")))
...
for yi,(lbl,val,ec,fc) in zip(y,rows):
    ...
    ax.text(val*1.03,yi,f"  {val:.2f} ms",va="center",ha="left",fontsize=13,fontweight="bold",color=ec)
ax.set_yticks(y); ax.set_yticklabels([r[0] for r in rows],fontsize=12)
ax.set_xlim(0, max(d["before_ms"],d["after_ms"])*1.35)
ax.set_xlabel("time per ak.argmin call  (ms)", fontsize=10.5)
...
ax.text(0.99,0.06,f"cuda.compute backend: {tag}   ·   same result",
        transform=ax.transAxes, ha="right", fontsize=10.5, color=GREEN, fontweight="bold")
