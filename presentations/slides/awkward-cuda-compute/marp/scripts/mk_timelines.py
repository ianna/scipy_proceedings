...
def draw_lane(ax,y,blocks,fc,ec,gap=3.0,label_dur=True,show_names=True):
    ...
    for name,dur in blocks:
        ...
        if show_names:
            ax.text(t+w/2,y+0.31,name if dur>8 else "",ha="center",va="center",fontsize=8.2,color=INK)
        if label_dur: ax.text(t+w/2,y-0.12,f"{dur:.0f}µs",ha="center",va="top",fontsize=7.6,color="#5a6b70")
        t+=w+gap
    return t

# ---- Figure 1: eager only (slide 4.1) ----
fig,ax=plt.subplots(figsize=(10,1.7)); ax.set_xlim(-4,AXMAX); ax.set_ylim(-0.5,1.0); ax.axis("off")
ax.set_title("Eager  |x| → sum   ·   3 kernels, 203 µs   (RTX 6000 Ada, nsys)",fontsize=12,fontweight="bold",color=RED,loc="left")
end=draw_lane(ax,0.15,eager,LBLUE,BLUE)
ax.annotate("intermediate y: 80 MB written → read back",xy=(150,0.95),ha="left",fontsize=8.5,color=RED,style="italic")
ax.plot([-4,AXMAX],[-0.02,-0.02],color=GREY,lw=0.8); ax.text(-4,-0.34,"GPU stream →  (µs)",fontsize=8,color="#7c8b90")
...
ax.text(-4,1.55,"eager",fontsize=9,color=RED,fontweight="bold")
draw_lane(ax,1.15,eager,LBLUE,BLUE)
ax.text(-4,0.62,"fused",fontsize=9,color=GREEN,fontweight="bold")
draw_lane(ax,0.22,fused,LGREEN,GREEN)
ax.annotate("the 150 µs abs pass + its DRAM round-trip disappear",xy=(70,0.05),ha="left",fontsize=8.6,color=GREEN,style="italic")
...
fig,ax=plt.subplots(figsize=(10,1.7)); ax.set_xlim(-4,AXMAX); ax.set_ylim(-0.5,1.0); ax.axis("off")
ax.set_title("torch.compile  |x| → sum   ·   2 Triton kernels, 27 µs   (implicit fusion)",fontsize=12,fontweight="bold",color=PURP,loc="left")
...
ax.set_title("|x| \u2192 sum on RTX 6000 Ada  \u2014  fusion collapses 3 kernels / 203 \u00b5s \u2192 2 / ~30 \u00b5s",fontsize=11.5,fontweight="bold",color=INK,loc="left")
def lane_lbl(y,txt,c): ax.text(-4,y+0.70,txt,fontsize=9.5,color=c,fontweight="bold")
